...
dates = mr.date_linspace(mr.now(), mr.now() + mr.hours(1.5), 200)
...
dcm_inertial_to_camera = np.swapaxes(np.swapaxes(np.stack((camera_x, camera_y, camera_z), axis=-1), 0, 2), 0, 1)
dcm_camera_to_chief = mr.stack_mat_mult_mat(dcm_inertial_to_chief, np.swapaxes(dcm_inertial_to_camera, 0, 1))
q_camera = mr.dcm_to_quat(dcm_camera_to_chief)
dcm_camera = mr.quat_to_dcm(q_camera)
...
ims = []
for imname in sorted([x for x in os.listdir("out") if '.png' in x], key=lambda x: int(x[5:-4])):
